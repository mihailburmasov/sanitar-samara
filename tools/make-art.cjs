...
// Запуск: REALESRGAN=.../realesrgan-ncnn-vulkan.exe node tools/make-art.cjs [имя ...]
...
(async () => {
  ...
  for (const name of Object.keys(ART)) {
    ...
    dims[name] = { w: d.width, h: d.height, mw: m.width, mh: m.height, kb: kb(`${name}.webp`), kb1200: kb(`${name}-1200.webp`), kbm: kb(`${name}-m.webp`), ...(ART[name].mode === 'strip' ? { strip: true } : {}) };
  }
  fs.writeFileSync(path.join(ROOT, 'data', 'hero.json'), JSON.stringify(dims, null, 2) + '\n');
