...
// Подписи иконок в админке (выбор иконки для карточек)
const ICON_LABELS = [
    'phone' => 'Телефон', 'telegram' => 'Telegram', 'max' => 'Сообщение', 'check' => 'Галочка', 'shield' => 'Щит', 'shield-check' => 'Щит с галочкой',
    'clock' => 'Часы', 'leaf' => 'Лист', 'star' => 'Звезда', 'pin' => 'Точка на карте', 'mail' => 'Почта', 'doc' => 'Документ', 'badge' => 'Награда',
    'spray' => 'Распылитель', 'drop' => 'Капля', 'calc' => 'Калькулятор', 'ruble' => 'Рубль', 'search' => 'Поиск', 'truck' => 'Грузовик',
    'clipboard' => 'Планшет', 'refresh' => 'Повтор', 'chat' => 'Чат',
    'tarakan' => 'Таракан', 'klop' => 'Клоп', 'blokha' => 'Блоха', 'muravei' => 'Муравей', 'mokritsa' => 'Мокрица', 'muha' => 'Муха', 'osa' => 'Оса',
    'komar' => 'Комар', 'klesh' => 'Клещ', 'tlya' => 'Тля', 'krysa' => 'Крыса', 'mysh' => 'Мышь', 'plesen' => 'Плесень', 'virus' => 'Вирус', 'fungus' => 'Грибок',
    'home' => 'Дом', 'sprout' => 'Росток', 'building' => 'Здание', 'utensils' => 'Общепит', 'warehouse' => 'Склад', 'buildings' => 'Дома', 'cross' => 'Медицина',
];

function sprite_svg(): string
{
    $s = '';
    foreach (ICONS as $id => $d) $s .= '<symbol id="i-' . $id . '" viewBox="0 0 32 32">' . $d . '</symbol>';
    return '<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" style="position:absolute" aria-hidden="true" focusable="false">' . $s . '</svg>';
}

function icon(string $id, string $cls = ''): string
{
    return '<svg class="ico' . ($cls !== '' ? ' ' . $cls : '') . '" aria-hidden="true" focusable="false"><use href="#i-' . $id . '"/></svg>';
}
