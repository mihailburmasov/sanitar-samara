...
(function () {
  ...
  function photoUrl(id, sm) { var p = PHOTOS && photoById(id); return p ? A.base + '/' + (sm !== false && p.sm ? p.sm : p.file) : ''; }
  function artUrl(k) { return A.base + '/images/hero/' + k + '-m.webp'; }
  ...
  F.photo = function (fd, v, ch) {
    ...
    } }), fd.required ? null : h('button', { class: 'btn btn--sm', type: 'button', text: 'Убрать', onclick: function () { cur = ''; upd(); ch(); } }));
    return { el: wrapFld(fd, box), get: function () { return cur; } };
  };
  ...
  F.service = function (fd, v, ch) { var s = serviceSelect(v, !v || !fd.required); s.addEventListener('change', ch); return { el: wrapFld(fd, s), get: function () { return s.value; } }; };
  ...
  F.herobg = function (fd, v, ch) {
    ...
    var pos = h('input', { type: 'text', class: 'inp', style: 'max-width:200px', placeholder: 'center' }); pos.value = cur.pos || '';
    pos.addEventListener('input', ch);
    function upd() { img.src = cur.art ? artUrl(cur.art) : photoUrl(cur.photo); cap.textContent = cur.art ? 'Фон «' + cur.art + '»' : 'Фото: ' + ((photoById(cur.photo) || {}).alt || cur.photo); }
    upd();
    var box = h('div', { class: 'pick' }, img, h('div', {},
      h('div', { style: 'display:flex;gap:6px;flex-wrap:wrap' },
        h('button', { class: 'btn btn--sm', type: 'button', text: 'Выбрать готовый фон', onclick: function () {
          var m;
      ...
      h('label', { class: 'fld__hint', style: 'display:block;margin-top:8px' }, 'Сдвиг кадра на компьютере (например, 70% center — показать правую часть): ', pos)));
    return { el: wrapFld(fd, box), get: function () { var o = clone(cur); delete o.pos; if (pos.value.trim()) o.pos = pos.value.trim(); return o; } };
  };
  ...
  routes[''] = function () {
    ...
    view().appendChild(h('div', {},
      ...
      h('div', { class: 'tiles' },
        tile('#/section/home', 'home', 'Главная', 'Заголовки, карточки, вопросы и ответы'),
        tile('#/services', 'spray', 'Услуги', 'Страницы услуг: тексты, цены, вопросы. Добавить новую'),
        tile('#/section/prices', 'ruble', 'Цены', 'Таблица цен и цена за сотку — меняются на всём сайте'),
        tile('#/photos', 'search', 'Фото', 'Фото работ и документы: загрузить, подписать'),
      ...
      h('div', { class: 'tiles' },
        ...
        tile('#/trash', 'refresh', 'Корзина', 'Удалённые услуги и фото можно вернуть'))));
  };
    ...
    return Promise.all([api('section?id=' + encodeURIComponent(id)), PHOTOS || loadPhotos()]).then(function (r) {
      ...
      view().appendChild(buildForm(d.fields, d.values, function (values) {
        return api('section', { body: { id: id, values: values } }).then(function (j) {
          ...
          if (id === 'company' || id === 'reviews' || id === 'prices') return refreshBoot().then(function () { rerender(); });
        });
      }));
    });
  };
  ...
  routes.photos = function (catKey) {
    return loadPhotos().then(function () {
      ...
      var grid = h('div', { class: 'ph' }, c.photos.map(function (p, i) {
        ...
        alt.addEventListener('input', changed); sel.addEventListener('change', changed);
        saveBtn.addEventListener('click', function () {
          api('photo/update', { body: { id: p.id, alt: alt.value, cat: sel.value } }).then(function (j) { toast(j.message); routes.photos(cat); }).catch(function (e) { toast(e.message, 'err'); });
        });
      ...
      view().innerHTML = '';
      add(view(), [h('p', { class: 'lead-p', text: 'Фото разложены по папкам. Выбрать фото для страницы можно в её форме (например, фото к тексту услуги или документ). Подпись к фото видят поисковики и незрячие посетители — пишите, что на фото.' }), tabs, drop,
        c.photos.length ? grid : h('div', { class: 'empty', text: 'В этой папке пока нет фото' })]);
    });
  };
        ...
        h('div', { class: 'tbl-wrap' }, h('table', { class: 'tbl' },
          ...
          h('tbody', {}, d.items.map(function (l) {
            return h('tr', {},
              ...
              h('td', {}, l.service ? h('div', { text: l.service }) : null, [['object', 'Объект'], ['address', 'Адрес'], ['callTime', 'Когда звонить']].map(function (k) { return l[k[0]] ? h('div', { text: k[1] + ': ' + l[k[0]] }) : null; }), l.message ? h('div', { text: l.message, style: 'color:#6b7078;white-space:pre-wrap' }) : null),
