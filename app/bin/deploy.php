...
function code_files(): array
{
    ...
    // статика сайта без фото клиента (images/foto — загрузки из админки, это контент, он живёт на сервере)
    $pub = rtrim(cfg('public_dir'), '/\\');
    foreach (local_files($pub, fn($r) => !str_starts_with($r, 'images/foto/')) as $r => $p) $files["@docroot/$r"] = $p;
    // размеры готовых фонов из images/hero — это описание кода, админка его не меняет
...
function content_files(): array
{
    ...
    foreach (local_files(rtrim(cfg('public_dir'), '/\\') . '/images/foto', fn($r) => preg_match('~^[^/]+\.webp$~', $r) === 1) as $r => $p) $files["@docroot/images/foto/$r"] = $p;
    return $files;
}
...
if ($cmd === 'pull') {
    ...
    $dirty = trim((string)shell_exec('git -C ' . escapeshellarg(ROOT_DIR) . ' status --porcelain -- data public/images/foto'));
    if ($dirty !== '' && !$force) { fwrite(STDERR, "В data/ или public/images/foto есть незакоммиченные изменения — сначала закоммитьте их:\n$dirty\n"); exit(1); }
    $c = ftp_open($D);
    $got = 0;
    $isData = fn($r) => preg_match('~\.json$~', $r) === 1 && !in_array($r, CODE_DATA, true);
    $isPhoto = fn($r) => preg_match('~^[^/]+\.webp$~', $r) === 1;
    $targets = [['data', cfg('data_dir'), $isData], ['@docroot/images/foto', rtrim(cfg('public_dir'), '/\\') . '/images/foto', $isPhoto]];
    ...
    foreach ($targets as [$rkey, $ldir, $keep]) {
        ...
        if (!$tree && $rkey !== 'data' && !local_files($ldir, $keep)) continue; // фото из админки ещё не загружали
