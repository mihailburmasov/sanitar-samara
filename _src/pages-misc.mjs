...
export function kontakty(C) {
  ...
  const p = C.phones;
  const map = C.address
    ? `${C.geo ? `<iframe class="map-frame" src="https://yandex.ru/map-widget/v1/?ll=${C.geo.lon},${C.geo.lat}&amp;z=16&amp;pt=${C.geo.lon},${C.geo.lat},pm2rdm" title="Офис на карте: ${L.esc(C.address)}" loading="lazy" allowfullscreen></iframe>` : ''}<a class="btn btn--ghost" href="https://yandex.ru/maps/?text=${encodeURIComponent(C.address)}" target="_blank" rel="noopener">${icon('pin')} Открыть на Яндекс Картах</a>`
...
<section class="section"><div class="container two-col">
  <div>
    ...
    <div class="prose" style="margin-top:24px">
      ...
      <p><b>E-mail:</b> ${C.email ? `<a href="mailto:${L.esc(C.email)}">${L.esc(C.email)}</a>` : L.val('EMAIL', C.email)}</p>
...
<p>${C.metrikaId ? 'Сайт использует файлы cookie для корректной работы и сбора статистики посещаемости с помощью Яндекс.Метрики (включая вебвизор). Метрика запускается только после вашего согласия в баннере.' : 'Сайт использует файлы cookie для корректной работы. Сервисы веб-аналитики на сайте сейчас не подключены.'} ${C.geo ? 'На странице «Контакты» встроена Яндекс Карта: сервис Яндекса может устанавливать собственные файлы cookie по своим правилам. ' : ''}Вы можете отключить cookie в настройках браузера, но некоторые функции сайта могут работать некорректно.</p>
