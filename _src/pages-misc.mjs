...
<p>${C.metrikaId ? 'Сайт использует файлы cookie для корректной работы, сохранения выбранной темы оформления и сбора статистики посещаемости с помощью Яндекс.Метрики (включая вебвизор). Метрика запускается только после вашего согласия в баннере.' : 'Сайт использует файлы cookie для корректной работы и сохранения выбранной темы оформления (светлой или тёмной). Сервисы веб-аналитики на сайте сейчас не подключены.'} Вы можете отключить cookie в настройках браузера, но некоторые функции сайта могут работать некорректно.</p>
